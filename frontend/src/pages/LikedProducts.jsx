...
const LikedProducts = () => {
  ...
  const handleUnlike = async (productId, e) => {
    ...
    try {
      await api.post(`/users/like/${productId}`);
    ...
    return (
      <div className="min-h-screen pt-32 bg-gray-50 flex items-center justify-center">
        <div className="relative w-16 h-16">
          ...
          <div className="absolute inset-0 rounded-full border-[3px] border-transparent border-t-red-500 border-r-red-500 animate-spin"></div>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-8xl mx-auto px-4 sm:px-6">
        <div className="flex items-center gap-4 mb-8">
          <div className="w-14 h-14 bg-red-100 text-red-500 rounded-2xl flex items-center justify-center shadow-inner">
            <svg className="w-7 h-7" fill="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" /></svg>
          </div>
          <div>
            <h1 className="text-3xl font-black text-gray-900 tracking-tight">Liked Products</h1>
            <p className="text-gray-500 font-medium text-sm mt-1">Products you have loved.</p>
          </div>
        </div>
        ...
        {products.length === 0 ? (
          <div className="text-center py-20 mb-5">
            <div className="w-20 h-20 bg-red-50 text-red-300 rounded-full flex items-center justify-center mx-auto mb-4">
            ...
            <p className="text-gray-500 font-medium">You haven't liked any products yet.</p>
            <button onClick={() => navigate('/products')} className="mt-4 px-6 py-2 bg-red-500 hover:bg-red-600 text-white font-bold rounded-xl transition-all shadow-md">Browse Products</button>
          </div>
        ) : (
            ...
            {currentProducts.map((product) => (
              ...
              >
                ...
                <div className="absolute top-4 right-4 z-10">
                  <button 
                    onClick={(e) => handleUnlike(product._id, e)}
                    className="w-10 h-10 rounded-full bg-red-500/90 hover:bg-red-600 backdrop-blur-md flex items-center justify-center text-white transition-all shadow-lg transform hover:scale-110"
